...
/**
 * Configure environment for Async Storage feature
 * (details described in issue https://github.com/eclipse/che/issues/15384)
 * This environment will allow backup on workspace stop event and restore on restart created earlier.
 * Will apply only in case workspace has attributes:
 *  - asyncPersist : true
 *  - persistVolumes : false
 * During provision will be created:
 *  - storage Pod
 *  - service for rsync connection via SSH
 *  - configmap, with public part of SSH key
...
public class AsyncStorageProvisioner {
  ...
  private static final String ASYNC_STORAGE = "async-storage";
  private static final String ASYNC_STORAGE_CONFIG = "async-storage-config";
  private static final String ASYNC_STORAGE_CLAIM = "async-storage-claim";
  ...
  public void provision(RuntimeIdentity identity) throws InfrastructureException {
    ...
    if (!isPvcExist) {
      PersistentVolumeClaim pvc = KubernetesObjectUtil.newPVC(ASYNC_STORAGE_CLAIM,
          accessMode, pvcQuantity);
    ...
    if (!isMapExist) {
      ConfigMap configMap = createConfigMap(namespace, configMapName, identity);
    ...
    boolean isServiceExist =
        oc.services()
            ...
            .stream()
            .anyMatch(
                (Predicate<Service>) service -> service.getMetadata().getName().equals(ASYNC_STORAGE));
   ...
   * @return
   */
  private ConfigMap createConfigMap(String namespace, String configMapName, RuntimeIdentity identity) {
  ...
  /**
   * Create storage Pod with container with mounted volume for storing project source backups,
   * SSH key and exposed port for rsync connection
   * @param namespace
   * @param configMap
   * @return
   */
